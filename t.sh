...
launch_qube() {
    ...
    # Clone template to RAM
    log_info "Cloning template to RAM (this may take a moment)..."
    qvm-clone --quiet -P "$POOL_NAME" "$TEMPLATE" "$QUBE_NAME" || {
        cleanup_qube "$QUBE_NAME" 1
        die "Failed to clone template"
    }
    
    # Make root volume read-only for extra security
    log_info "Setting root volume to read-only..."
    qvm-volume config "${QUBE_NAME}:root" rw False 2>/dev/null || true
    
    # Configure qube properties
    log_info "Configuring qube properties..."
    
    qvm-prefs "$QUBE_NAME" include_in_backups False
    qvm-prefs "$QUBE_NAME" template_for_dispvms False
    qvm-prefs "$QUBE_NAME" memory "$MEMORY"
    qvm-prefs "$QUBE_NAME" label "$LABEL"
