...
launch_qube() {
    ...
    # Always set 'pool'. Additionally set per-volume pools if supported.
    local create_props=()
    create_props+=( --property "pool=${POOL_NAME}" )
    if vm_has_pref "$TEMPLATE" "root_pool"; then
        create_props+=( --property "root_pool=${POOL_NAME}" )
    fi
    if vm_has_pref "$TEMPLATE" "private_pool"; then
        create_props+=( --property "private_pool=${POOL_NAME}" )
    fi
    if vm_has_pref "$TEMPLATE" "volatile_pool"; then
