...
init_lock_fd() {
    ...
    # Validate XDG_RUNTIME_DIR itself (fail closed)
    [[ -n "${XDG_RUNTIME_DIR:-}" ]] || die "XDG_RUNTIME_DIR is not set"
    [[ ! -L "$XDG_RUNTIME_DIR" ]] || die "Security: XDG_RUNTIME_DIR is a symlink: $XDG_RUNTIME_DIR"
    [[ -d "$XDG_RUNTIME_DIR" ]] || die "Security: XDG_RUNTIME_DIR is not a directory: $XDG_RUNTIME_DIR"

    local x_uid x_mode
    x_uid="$(stat -Lc '%u' -- "$XDG_RUNTIME_DIR" 2>/dev/null)" || die "Security: cannot stat XDG_RUNTIME_DIR"
    x_mode="$(stat -Lc '%a' -- "$XDG_RUNTIME_DIR" 2>/dev/null)" || die "Security: cannot stat XDG_RUNTIME_DIR"
    [[ "$x_uid" == "$uid" ]] || die "Security: XDG_RUNTIME_DIR owner mismatch (expected uid=$uid, got $x_uid)"
    [[ "$x_mode" == "700" ]] || die "Security: XDG_RUNTIME_DIR mode must be 700 (got $x_mode): $XDG_RUNTIME_DIR"

    # Create lock file if missing (without truncating/replacing an existing inode)
    if [[ ! -e "$lock_fd_file" ]]; then
        local tmp
        (
            umask 077
            tmp="$(mktemp -p "$XDG_RUNTIME_DIR" ram-disposable.lockfd.XXXXXX)" || exit 1
            chmod 600 -- "$tmp" || exit 1
            # Hardlink into final name atomically; fails if destination appears (incl. symlink) meanwhile.
            ln -T -- "$tmp" "$lock_fd_file" 2>/dev/null || exit 1
            rm -f -- "$tmp" 2>/dev/null || true
        ) || die "Cannot create lock file safely: $lock_fd_file"
    fi

    # Validate lock file (fail closed on symlink, non-regular, hardlinks)
    [[ ! -L "$lock_fd_file" ]] || die "Security: lock file path is a symlink: $lock_fd_file"
    [[ -f "$lock_fd_file" ]] || die "Security: lock file is not a regular file: $lock_fd_file"

    local f_uid f_mode f_links
    f_uid="$(stat -Lc '%u' -- "$lock_fd_file" 2>/dev/null)" || die "Security: cannot stat lock file"
    f_mode="$(stat -Lc '%a' -- "$lock_fd_file" 2>/dev/null)" || die "Security: cannot stat lock file"
    f_links="$(stat -Lc '%h' -- "$lock_fd_file" 2>/dev/null)" || die "Security: cannot stat lock file"
    [[ "$f_uid" == "$uid" ]] || die "Security: lock file owner mismatch (expected uid=$uid, got $f_uid)"
    [[ "$f_links" == "1" ]] || die "Security: lock file has hardlinks (link count=$f_links): $lock_fd_file"
    if [[ "$f_mode" != "600" ]]; then
        chmod 600 -- "$lock_fd_file" 2>/dev/null || die "Security: cannot set lock file mode to 600: $lock_fd_file"
    fi

    # Defense-in-depth: ensure the lock file resolves *inside* XDG_RUNTIME_DIR
    command -v realpath >/dev/null 2>&1 || die "Security: realpath is required for lockfile validation"
    local base_real lock_real
    base_real="$(realpath -e -- "$XDG_RUNTIME_DIR" 2>/dev/null)" || die "Security: cannot resolve XDG_RUNTIME_DIR"
    lock_real="$(realpath -e -- "$lock_fd_file" 2>/dev/null)" || die "Security: cannot resolve lock file path"
    # base_real itself is a directory; require lock_real to be a child path
    [[ "$lock_real" == "$base_real/"* ]] || die "Security: lock file resolves outside XDG_RUNTIME_DIR: $lock_real"

    # Open without truncation (">" would truncate; we only need an fd for flock)
    exec 200<>"$lock_fd_file" || die "Cannot open lock file descriptor"
    LOCK_FD=200
}
...
vm_has_pref() {
    # Usage: vm_has_pref <vmname> <prefname>
    local vm="$1"
    local pref="$2"
    qvm-prefs "$vm" 2>/dev/null | awk '{print $1}' | grep -Fxq -- "$pref"
}

set_vm_pref_if_supported() {
    # Usage: set_vm_pref_if_supported <vmname> <prefname> <value>
    local vm="$1"
    local pref="$2"
    local value="$3"
    vm_has_pref "$vm" "$pref" && qvm-prefs "$vm" "$pref" "$value" 2>/dev/null || true
}
...
ensure_qube_on_ram_pool() {
    ...
    # Verify volumes are safe (in RAM pool, or read-only)
    local vol cur_pool vol_rw
    for vol in root private volatile; do
        if qvm-volume info "${qube}:${vol}" &>/dev/null; then
            ...
            if [[ "$cur_pool" != "$pool" ]]; then
                # Root is OK if read-only (writes go to volatile which is in RAM)
                if [[ "$vol" == "root" ]]; then
                    vol_rw="$(qvm-volume info "${qube}:${vol}" rw 2>/dev/null || echo "True")"
                    if [[ "$vol_rw" == "False" ]]; then
                        log_info "Volume ${qube}:root is read-only, safe on disk"
                        continue
                    fi
                    # Not read-only -> may write COW data to disk; warn but continue (user requested)
                    log_warn "UNSAFE: Volume ${qube}:root is on pool '${cur_pool}', expected '${pool}'. Continuing anyway."
                    continue
                fi
                die "Volume ${qube}:${vol} is on pool '${cur_pool}', expected '${pool}'. Data may leak to disk!"
            fi
            log_debug "Volume ${qube}:${vol} verified on pool ${pool}"
        fi
    done
    
    log_info "All volumes verified safe"
}
...
launch_qube() {
    ...
    sudo mount --types tmpfs \
        --options "size=${TEMPSIZE},mode=700,nodev,nosuid,noexec" \
        "$POOL_NAME" \
    ...
    # Try to force RAM_TEMPLATE default pools to our RAM-backed pool (if prefs exist in this Qubes version)
    set_vm_pref_if_supported "$RAM_TEMPLATE" "root_pool"     "$POOL_NAME"
    set_vm_pref_if_supported "$RAM_TEMPLATE" "private_pool"  "$POOL_NAME"
    set_vm_pref_if_supported "$RAM_TEMPLATE" "volatile_pool" "$POOL_NAME"
    ...
    # Force volumes to be created in the RAM-backed pool (Qubes version dependent).
    # Do NOT set 'pool' (some Qubes versions don't have such VM property).
    # Set per-volume pools if supported.
    local create_props=()
    if vm_has_pref "$RAM_TEMPLATE" "root_pool"; then
        create_props+=( --property "root_pool=${POOL_NAME}" )
    fi
    if vm_has_pref "$RAM_TEMPLATE" "private_pool"; then
        create_props+=( --property "private_pool=${POOL_NAME}" )
    fi
    if vm_has_pref "$RAM_TEMPLATE" "volatile_pool"; then
        create_props+=( --property "volatile_pool=${POOL_NAME}" )
    fi
    ...
    set +e
    qvm-run --no-shell "$QUBE_NAME" -- "${COMMAND_ARGS[@]}"
    run_exit_code=$?
    set -e
