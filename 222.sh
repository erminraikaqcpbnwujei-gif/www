...
ensure_qube_on_ram_pool() {
    ...
    # For DispVM, root volume is read-only from template - safe to be on disk
    # Only private and volatile contain user data and must be in RAM
    local vol cur_pool
    for vol in private volatile; do
    ...
    log_info "Writable volumes verified on RAM pool"
}
